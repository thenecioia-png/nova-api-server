...
════════════════════════════════════════
DIRECTIVA FINAL — ESTO ANULA TODO LO DEMÁS
════════════════════════════════════════
SI EL MENSAJE PIDE ALGO EN PC O NAVEGADOR → escribe 1 línea corta ("Capturando pantalla...") y llama ejecutar_en_pc. Sin más texto.
SI EL MENSAJE PIDE INFORMACIÓN ACTUAL → escribe 1 línea corta ("Buscando...") y llama buscar_web. Sin más texto.
ANTES DE CADA HERRAMIENTA: UNA sola línea de narración. Una. No un párrafo, no una lista.
NUNCA un plan completo antes de actuar. NUNCA "aquí te explico lo que haré".
ACTÚAS → REPORTAS. Denison siempre sabe qué estás haciendo porque lo narras brevemente.`;
}
...
router.post("/asistente/ia", async (req, res) => {
  ...
  try {
    ...
    // ── Late system injection: forces action-first when bot is online ─────────
    // This is placed right before the user message so it's the freshest context
    if (botOnline) {
      mensajes.push({
        role: "system" as const,
        content: `RECORDATORIO CRÍTICO [${new Date().toISOString()}]: El bot PC está ONLINE. Si el siguiente mensaje implica cualquier acción en PC o navegador, tu PRIMERA respuesta DEBE ser una llamada a ejecutar_en_pc — NADA de texto antes. Para información actual, llama buscar_web primero. CERO explicaciones previas. ACCIÓN DIRECTA.`,
      });
    }

    // ── Auto-screenshot: inject live desktop view when bot is online ──────────
    // Only when NOT sending an image file (no need to double-inject)
    let autoScreenBase64: string | null = null;
    if (botOnline && !(archivoBase64 && archivoTipo?.startsWith("image/"))) {
      // Check if we already have a fresh screenshot (< 8s) before showing "capturing..." status
      const isFreshOnDisk = fs.existsSync(SCREENSHOT_PATH) &&
        (Date.now() - fs.statSync(SCREENSHOT_PATH).mtimeMs) < 8000;
      if (!isFreshOnDisk) {
        sse({ tipo: "status", contenido: "📸 Capturando vista actual del escritorio..." });
      }
      autoScreenBase64 = await autoScreenshotFast();
      if (autoScreenBase64) {
        sse({ tipo: "status", contenido: "👁️ Pantalla lista — N.O.V.A. ya puede ver tu escritorio" });
      }
    }
    ...
    if (archivoBase64 && archivoTipo?.startsWith("image/")) {
      ...
      mensajes.push({
        ...
        content: [
          { type: "image_url", image_url: { url: `data:image/png;base64,${autoScreenBase64}`, detail: "high" } },
          { type: "text", text: `[🖥️ SCREENSHOT DEL ESCRITORIO — ${new Date().toLocaleTimeString()}]\n🚫 INSTRUCCIÓN CRÍTICA: Esta imagen YA ES el screenshot actual. PROHIBIDO llamar ejecutar_en_pc con tipo "screenshot" — ya tienes la vista completa. Pasa DIRECTAMENTE a responder o ejecutar la tarea sin tomar screenshot.\n\nMensaje de Denison: ${mensaje}` },
        ],
      });
    } else {
